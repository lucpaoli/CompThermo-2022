...
$$\begin{gather}
p(T_0, v^L) - p(T_0, v^V) = 0\\
\mu(T_0, v^L) - \mu(T_0, v^V) = 0
\end{gather}$$

#### Pressure specification 

$$\begin{gather}
T(P_0, v^L) - T(P_0, v^V) = 0\\
\mu(P_0, v^L) - \mu(P_0, v^V) = 0
\end{gather}$$
...
begin
	...
	(p_sat, Vl, Vv) = solve_sat_p(model, T, V0; itersmax=100, abstol=1e-6)
end
...
md"""
The main improvement, however, can come from properly scaling our equations to begin with. By using the scaling factors presented below, our initial condition number falls to $\sim 10^2$.

Because of this, our convergence is improved and are able to converge our saturation solver to a tolerance of $\sim 10^{-13}$.
"""

# ╔═╡ 4caeab94-6181-416c-9917-e56c0be6d0ff
@htl("""
		<table>
...
let
	function sat_p_objective_scaled(model::Clapeyron.ABCubicModel, T, V)
		R = 8.314
...
begin
	plot(title="PT plot for water", xlabel="T (K)", ylabel="P (Pa)", framestyle=:box, tick_direction=:out, grid=:off)
...
begin
	...
	function critical_objective!(model, F, x)
		p_scale = Clapeyron.p_scale(model)
		
		V = x[1]
		T = x[2]
		p(V) = pressure(model, V, T)/p_scale
	...
	V03 = 5.24e-5
	T03 = 547.0
	Tscale = Clapeyron.T_scale(model2)
	x0 = [log10(V03), T03/Tscale]

	# Apply scaling factors; For this just use the current 0 value
	res = nlsolve((F, x) -> critical_objective!(model2, F, [exp10(x[1]), x[2]*Tscale]), x0, autodiff = :forward, xtol=1e-2)
	Vc, Tc = exp10(res.zero[1]), res.zero[2]*Tscale
end
